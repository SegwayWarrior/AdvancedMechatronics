...
void i2c_set_pin(unsigned char write_address, unsigned char regist, unsigned char value){
    // Write a pin value to a specific register
    i2c_master_start();
...
unsigned char i2c_read_pin(unsigned char read_address, unsigned char regist){
    // Read a value from a specific register
    unsigned char value;
    i2c_master_start();
    i2c_master_send(read_address & 0b11111110); // change to write address
    i2c_master_send(regist);
    i2c_master_restart();
    i2c_master_send(read_address|0b00000001);
    value = i2c_master_recv();
    i2c_master_ack(1);  // Only send one byte
    i2c_master_stop();
    return value;
}
